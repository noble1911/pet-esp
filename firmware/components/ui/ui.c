...
static void make_scene(lv_obj_t *scr)
{
    ...
    // 2x integer NEAREST upscale — matches the pet's 2x rendering for
    // consistent pixel-art chunkiness. 184x152 -> 368x304 fills the
    // scene area exactly.
    lv_image_set_scale(bg, 256 * 2);
    // Position so the scaled image's top-left sits at (0, SCENE_TOP_Y).
    // LVGL scales around the centre point, so the object's x/y is its
    // CENTRE-PIVOT location: we offset by half the scaled extent.
    lv_obj_set_pos(bg, 0 - (184 / 2), SCENE_TOP_Y - (152 / 2));
