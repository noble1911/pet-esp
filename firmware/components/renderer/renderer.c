...
void renderer_init(void)
{
    ...
    // BSP 1.1.3 registers this QSPI panel as an RGB display, signalling flush
    // completion before SPI DMA finishes. Its PSRAM draw buffer also requires
    // a temporary internal DMA allocation on every transfer. Wi-Fi can exhaust
    // that allocation. Own the port setup: reserve a small DMA buffer at boot
    // and let the SPI completion callback release it only after transmission.
    lvgl_port_cfg_t port_cfg = ESP_LVGL_PORT_INIT_CONFIG();
    ESP_ERROR_CHECK(lvgl_port_init(&port_cfg));
    esp_lcd_panel_handle_t panel;
    esp_lcd_panel_io_handle_t panel_io;
    bsp_display_config_t panel_cfg = {0};
    ESP_ERROR_CHECK(bsp_display_new(&panel_cfg, &panel, &panel_io));
    ...
    ESP_LOGI(TAG, "QSPI display: reserved %u-byte DMA buffer; completion-synchronised flush",
             BSP_LCD_H_RES * 16 * 2);
