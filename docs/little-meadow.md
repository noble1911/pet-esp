...
The subsequent approved concept replaces the vector bunny with a sprout creature and an illustrated indoor room. `pixel_pet.c` draws the animated 56 × 56 sprite directly into ARGB8888 pixels; a saved colour gene selects six palettes and growth retains the five existing milestones. Expressions cover idle/blink, joy, eating (held apple) and sleeping under a patchwork blanket. Two separately generated room assets are packed into 184 × 152 RGB565 and displayed 2× with filtering off. Asset provenance and full prompts are in `art/pixel-v1/README.md`.

The UI keeps the existing activities, navigation targets and save schema. Pixel titles use UNSCII; navigation symbols use Montserrat because UNSCII lacks those glyphs. The old source lives at Git tag `vector-art-v1` and its four verified firmware images plus restore script live in `backup/vector-v1/`. Restoring that bundle skips the NVS partition so progress survives either direction.
