...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'docs/previews/five-characters-v1'
p=argparse.ArgumentParser();p.add_argument('captures',type=Path);args=p.parse_args();OUT.mkdir(parents=True,exist_ok=True)
...
out=Image.new('RGB',(12*144,5*174),BG);d=ImageDraw.Draw(out)
for c in range(5):
...
out=Image.new('RGB',(14*144,5*166),BG);d=ImageDraw.Draw(out)
for c in range(5):
...
for name,face,pose in scenes:
 out=Image.new('RGB',(5*216,248),BG);d=ImageDraw.Draw(out)
 for c,char in enumerate(chars):
  out.paste(frame(c,face,pose),(c*216,24));d.text((c*216+10,5),char['name']+' / '+name,fill=INK)
 frames.append(out)
frames[0].save(OUT/'five-friends.png');frames[0].save(OUT/'five-friends.gif',save_all=True,append_images=frames[1:],duration=[900,650,160,650,650,650,400,400,800,800,600,600],loop=0,disposal=2)
